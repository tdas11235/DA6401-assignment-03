...
class SeqModel(nn.Module):
    ...
    def forward(self, src, tgt, teacher_forcing_ratio=0.0):
        ...
        attentions = torch.zeros(
            batch_size, max_len, encoder_outputs.size(1)).to(self.device)
        for t in range(max_len):
            ...
            input_token = tgt[:, t].unsqueeze(1) if torch.rand(
                1).item() < teacher_forcing_ratio else top1
                ...
                for seq, score, hidden_state in sequences:
                    ...
                    for i in range(beam_width):
                        ...
                        all_candidates.append(
                            (new_seq, new_score, hidden_clone))
    ...
    def greedy_decode_batch(self, src_batch, max_len=30):
        ...
        with torch.no_grad():
            ...
            decoded_tokens = torch.full(
                (batch_size, max_len + 1), self.tgt_vocab['<pad>'], dtype=torch.long, device=self.device)
